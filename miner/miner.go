...
func (miner *Miner) Pending() (*types.Block, types.Receipts, *state.StateDB) {
	if miner.chainConfig.IsOptimism() && !miner.config.RollupComputePendingBlock {
		header := miner.chain.CurrentHeader()
		headBlock := miner.chain.GetBlock(header.Hash(), header.Number.Uint64())
		receipts := miner.chain.GetReceiptsByHash(header.Hash())
		stateDb, err := miner.chain.StateAt(header.Root)
		if err != nil {
			// TODO: Log
			return nil, nil, nil
		}

		return headBlock, receipts, stateDb
	}
...
func (miner *Miner) getPending() *newPayloadResult {
	if miner.chainConfig.IsOptimism() && !miner.config.RollupComputePendingBlock {
		return nil // when not computing the pending block, there is never a pending state
	}
